...
//Ter a altura da árvore de forma dinânmica auxilia na formação da matriz, para a craição do desenho da árvore.
...
void imprimir(NoArv *raiz) {
	...
	//Sempre que não haver um valor de inserção(-1), um espaço em branco será dado para a formação do desenho.
    for (i = 0; i < linha; i++) {
        for (j = 0; j < coluna; j++) {
            if (arvore[i][j] == -1) {
                printf("   ");
